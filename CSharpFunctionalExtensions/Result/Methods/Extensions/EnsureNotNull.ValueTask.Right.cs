...
namespace CSharpFunctionalExtensions.ValueTasks
{
    public static partial class ResultExtensions
    {
        public static ValueTask<Result<T>> EnsureNotNull<T>(this Result<T?> result, Func<ValueTask<string>> errorFactory)
            where T : class
        {
            return result.Ensure(value => ValueTask.FromResult(value != null), _ => errorFactory()).MapAsync(value => value!);
        }

        public static ValueTask<Result<T>> EnsureNotNull<T>(this Result<T?> result, Func<ValueTask<string>> errorFactory)
            where T : struct
        {
            return result.Ensure(value => ValueTask.FromResult(value != null), _ => errorFactory()).MapAsync(value => value!.Value);
        }
